-- [[ ROCKET ADMIN V42: SERVER-SYNCED ]] --
...
-- 2. VOID PROTECTION V3 (Physics-Based)
RunService.Heartbeat:Connect(function()
    ...
    if root and hum then
        -- Trigger if too low OR if velocity is indicating a massive "fling"
        if root.Position.Y < -35 or root.AssemblyLinearVelocity.Magnitude > 400 then
            root.AssemblyLinearVelocity = Vector3.new(0, 0, 0)
            root.CFrame = CFrame.new(root.Position.X, 100, root.Position.Z)
            -- Brief invulnerability to state trips
...
task.spawn(function()
    while true do
        ...
        task.wait(0.2) -- Slower to save performance
    end
end)

-- 4. GIVE ALL (Optimized)
task.spawn(function()
...
local function createBtn(txtOn, txtOff, y, getVal, setVal)
    ...
    local function update()
        ...
        b.BackgroundColor3 = active and Color3.fromRGB(0, 100, 150) or Color3.fromRGB(40, 40, 45)
    end
    ...
    for _, p in pairs(Players:GetPlayers()) do
        if p ~= player then
            ...
            b.MouseButton1Click:Connect(function()
                ...
                -- Heartbeat runs at server physics rate (60Hz)
