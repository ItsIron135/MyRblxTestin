...
-- 2. UNIVERSAL SHIELD (VOID PROTECTION)
...
-- 4. PHASE TELEPORT + GHOST RESPAWN LOGIC
local targetIndex = 1
RunService.Heartbeat:Connect(function()
    ...
    if #targets > 0 then
        ...
        local tPlayer = targets[targetIndex]
            ...
            for _, r in pairs(workspace:GetChildren()) do
                if r:IsA("BasePart") and (r.Name == "Rocket" or r.Name == "Projectile") then
                    if (r.Position - root.Position).Magnitude < 15 then
                        r.CFrame = CFrame.lookAt(r.Position, tRoot.Position)
                        r.AssemblyLinearVelocity = (tRoot.Position - r.Position).Unit * 300
                    end
                end
            end
        else
