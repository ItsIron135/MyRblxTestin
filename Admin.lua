...
local cam, PG =
    workspace.CurrentCamera, LP:FindFirstChild("PlayerGui") or LP:WaitForChild("PlayerGui", 5)
if PG:FindFirstChild("StealerUI") then
    PG.StealerUI:Destroy()
end
...
---------------------------------------------------------
local env = getgenv()
env.desyncActive, env.fakeCF, env.realCF =
    env.desyncActive or false, env.fakeCF or CFrame.new(), env.realCF or CFrame.new()
...
local NoclipActive, GhostTouchActive, GiveDroppedGearActive, IsStackingActive, isGodMode, InfSupernovaActive, isAntiStealing =
    false, false, false, false, false, false, false
local IsAntiLaser, AntiVoidActive, GiveAllBtn = false, true, nil
local NullDesyncActive, nullGhostOffset, nullPartOffsets, nullCamPart, nullSavedCF, nullWeldOverride =
    true, Vector3.new(), {}, nil, nil, nil
...
local SKillActive, SKillTargets, SKillLoops, SKillTrackers, ActiveSKillSwords =
    false, {}, {}, {}, {}
local VoidKillActive, VoidKillTargets, VoidKillLoops, VoidKillTrackers, ActiveVoidKillSwords =
    false, {}, {}, {}, {}
...
local AntiPickupActive, hiddenTools, antiPickupConnection, antiPickupCharConn, antiPickupBpConn =
    false, {}, nil, nil, nil
...
local MF = set(
    ...
    {
        Size = UDim2.new(0, 240, 0, 610),
        Position = UDim2.new(0.85, -120, 0.5, -200),
...
local SF = set(
    Instance.new("ScrollingFrame", MF),
    {
        Size = UDim2.new(1, 0, 1, -440),
...
local SP_Panel = set(
    Instance.new("Frame", SG),
    {
        ...
        Position = UDim2.new(0.5, -100, 0.5, -200),
...
set(
    Instance.new("TextLabel", SP_Panel),
    {
        Size = UDim2.new(1, -30, 0, 30),
        BackgroundTransparency = 1,
        Text = "PLAYER VIEWER",
...
local SP_Close = set(
    Instance.new("TextButton", SP_Panel),
    {
        Size = UDim2.new(0, 30, 0, 30),
        Position = UDim2.new(1, -30, 0, 0),
...
local SP_Scroll = set(
    Instance.new("ScrollingFrame", SP_Panel),
    {
...
local function startSpectate(p)
    ...
    if not originalCamSubject then
...
local function populateSpectateMenu()
    for _, c in ipairs(SP_Scroll:GetChildren()) do
    ...
    for _, p in ipairs(P:GetPlayers()) do
        if p ~= LP then
            local btn = set(
                Instance.new("TextButton", SP_Scroll),
                {
                    Size = UDim2.new(1, -10, 0, 30),
                    BackgroundColor3 = (savedPlayer == p) and Color3.fromRGB(0, 150, 100)
        ...
        if b:IsA("TextButton") then
            ...
            if p then
                ...
                if RocketSpamActive and RocketTargets[p.UserId] then
                    c = Color3.fromRGB(180, 40, 40)
                elseif VoidKillActive and VoidKillTargets[p] then
                    c = Color3.fromRGB(40, 150, 160)
                elseif SKillActive and SKillTargets[p] then
                    c = Color3.fromRGB(40, 110, 150)
                elseif SpawnCloneActive and SpawnCloneTargets[p] then
                    c = Color3.fromRGB(40, 120, 40)
                elseif StealArmActive and StealArmTargets[p] then
                    c = Color3.fromRGB(140, 90, 20)
                elseif WeldKillActive and WeldKillTargets[p] then
                    c = Color3.fromRGB(180, 100, 40)
                elseif FireInterestActive and FireInterestTargets[p] then
                    c = Color3.fromRGB(200, 50, 200)
                end
...
local function createToggle(col, yPos, text, activeCol, init, action)
    local btn = makeBtn(
        ...
        UDim2.new(col, 0, 1, yPos),
        init and activeCol or Color3.fromRGB(50, 50, 50)
    )
    btn.TextSize = 11
    btn.MouseButton1Click:Connect(function()
        local s = action()
        btn.Text, btn.BackgroundColor3 =
            text .. (s and ": ON" or ": OFF"), s and activeCol or Color3.fromRGB(50, 50, 50)
    end)
    return btn
end
...
local function hideTool(t)
    if
        t:IsA("Tool")
        and t.Parent
        and not t.Parent:FindFirstChild("Humanoid")
        and not t.Parent:IsA("Backpack")
    then
...
local function restoreTools()
    for t, p in pairs(hiddenTools) do
        if t then
            pcall(function()
                t.Parent = p or workspace
            end)
        end
    end
...
local function attachTool(tPlayer, tool, loopsMap)
    ...
    for _, part in pairs(tool:GetDescendants()) do
        if part:IsA("BasePart") then
            part.Massless, part.CanCollide, part.CustomPhysicalProperties =
                true, false, PhysicalProperties.new(0, 0, 0, 0, 0)
        end
    end
    local handle, rightArm =
    ...
    if handle and rightArm then
        ...
        if hum then
            for _, t in pairs(hum:GetPlayingAnimationTracks()) do
                if
                    t.Name:lower():find("tool")
                    or (t.Animation and tostring(t.Animation.AnimationId):find("507768375"))
                then
                    t:Stop()
                end
            end
...
local function toggleTargetSkill(btn, t, swordName, targetMap, loopMap, trackerMap, activeMap)
    if targetMap[t] then
        clearToolTargets({ [t] = true }, loopMap, trackerMap, activeMap)
        targetMap[t] = nil
    else
        ...
        if char then
            for _, o in pairs(char:GetChildren()) do
                if o.Name == swordName then
                    table.insert(allSwords, o)
                end
            end
        end
        if bp then
        ...
        if sword then
            ...
            trackerMap[t.Name] = t.CharacterAdded:Connect(function()
            ...
            task.delay(1, function()
                if btn and btn.Parent and not targetMap[t] then
                    btn.Text = old
                    refreshPlayerColors()
                end
            end)
...
local function consistentWeldTPGive(tp)
    ...
    local tRoot = tChar
        and (tChar:FindFirstChild("HumanoidRootPart") or tChar:FindFirstChild("Torso"))
    local char = LP.Character
    if not tRoot or not char then
        return
    end
            ...
            if tool and tool:IsA("Tool") then
                ...
                for _, j in ipairs(obj:GetChildren()) do
                    if
                        ...
                        or j:IsA("Motor6D")
                        or j:IsA("JointInstance")
                    then
                        if (j.Part0 and myParts[j.Part0]) or (j.Part1 and myParts[j.Part1]) then
                            linked = true
                            break
                        end
                    end
                end
                if not linked and tool:IsDescendantOf(char) then
                    linked = true
                end
                if linked then
                    table.insert(handles, obj)
                end
            end
        end
    end
        ...
        while tick() - s < 1.2 do
            ...
            for _, h in ipairs(handles) do
                if h and h.Parent then
                    pcall(function()
                        h.CFrame = tpRoot.CFrame
                        h.AssemblyLinearVelocity = Vector3.new()
                    end)
                    pcall(function()
                        if firetouchinterest then
                            firetouchinterest(h, tpRoot, 0)
                            firetouchinterest(h, tpRoot, 1)
                        end
                    end)
                end
            end
...
local function executeSpectralNuke(t, btn)
    ...
    if not c or not h or not hrp or not targetTorso then
        return
    end
    ...
    for _, i in ipairs(bp:GetChildren()) do
        if i.Name == "SpectralSword" and not UsedSwords[i] then
            sS = i
            break
        end
    end
...
local function giveAllFindCarrot()
    ...
    if bp then
        local c = bp:FindFirstChild("Carrot")
        if c then
            return c
        end
    end
    if char then
        local c = char:FindFirstChild("Carrot")
        if c then
            return c
        end
    end
...
local function runGiveAll()
    ...
    task.spawn(function()
        while giveAllRunning do
            giveAllFireOnce()
            if giveAllFindCarrot() then
                break
            end
            task.wait(GIVE_ALL_NORMAL_DELAY)
        end

        if not giveAllRunning then
            return
        end
...
local function stopNullDesync()
    ...
    if nullDesyncLoop then
        nullDesyncLoop:Disconnect()
        nullDesyncLoop = nil
    end
    if nullCamPart then
        nullCamPart:Destroy()
    ...
    if char and hum and root then
        ...
        if nullSavedCF then
            root.CFrame, root.AssemblyLinearVelocity, root.AssemblyAngularVelocity =
                nullSavedCF, Vector3.new(), Vector3.new()
        end
    end
    nullPartOffsets = {}
end
...
local function startNullDesync(char)
    ...
    nullDesyncLoop = RS.Heartbeat:Connect(function(dt)
        ...
        if not char or not currentRoot or not nullCamPart then
            return
        end
        if getgenv().IsDroppingGears then
            return
        end
        local lookCF, moveDir = cam.CFrame, Vector3.new()
        if UIS:IsKeyDown(Enum.KeyCode.W) then
            moveDir += lookCF.LookVector
        end
        if UIS:IsKeyDown(Enum.KeyCode.S) then
            moveDir -= lookCF.LookVector
        end
        if UIS:IsKeyDown(Enum.KeyCode.A) then
            moveDir -= lookCF.RightVector
        end
        ...
        if moveDir.Magnitude > 0 then
            nullGhostOffset += (moveDir.Unit * (FlySpeed * dt))
        end
        ...
        for part, offset in pairs(nullPartOffsets) do
            if part and part.Parent then
                part.CFrame, part.AssemblyLinearVelocity, part.AssemblyAngularVelocity =
                    ghostRootCF * offset, Vector3.new(), Vector3.new()
            end
        end
...
local function stopDesync()
    ...
    pcall(function()
        RS:UnbindFromRenderStep("DesyncCameraFix")
    end)
    local root = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
    if root then
        root.CFrame = env.realCF
        for _, i in ipairs(root:GetChildren()) do
            if i:IsA("BodyGyro") or i:IsA("BodyPosition") then
                i:Destroy()
            end
        end
    end
...
UIS.InputBegan:Connect(function(input, gp)
    ...
    elseif input.KeyCode == Enum.KeyCode.RightBracket then
        if SP_Panel.Visible then
            ...
            if
                savedPlayer
                and savedPlayer.Parent
                and savedPlayer.Character
                and not isSpectating
            then
    ...
    elseif input.KeyCode == Enum.KeyCode.LeftBracket then
        ...
        if char and bp and hum then
            ...
            if equipped then
                equipped.Parent = bp
                if BoneSwordTouchLoop then
                    BoneSwordTouchLoop:Disconnect()
                    BoneSwordTouchLoop = nil
                    print("[BONESWORD] Touch stopped")
                end
            else
                ...
                if unequipped then
                    ...
                    BoneSwordTouchLoop = RS.Heartbeat:Connect(function()
                        ...
                        for _, p in pairs(P:GetPlayers()) do
                            if p ~= LP and p.Character then
                                local tp = p.Character:FindFirstChild("Torso")
                                    or p.Character:FindFirstChild("HumanoidRootPart")
                                if tp then
                                    pcall(function()
                                        firetouchinterest(handle, tp, 0)
                                        task.wait()
                                        firetouchinterest(handle, tp, 1)
                                    end)
                                end
                            end
                        end
...
RS.Heartbeat:Connect(function()
    ...
    if not myTorso or not char then
        return
    end
    for _, p in pairs(P:GetPlayers()) do
        if p ~= LP and p.Character then
            local tt = p.Character:FindFirstChild("Torso")
            ...
            if
                ...
                and th
                and th.Health > 0
                and (myTorso.Position - tt.Position).Magnitude <= KillAuraRange
            then
                if not KillAuraDebounces[p] then
                    KillAuraDebounces[p] = true
                    local equipped = 0
                    for _, t in ipairs(char:GetChildren()) do
                        if t.Name == "TriLaserGun" then
                            equipped += 1
                        end
                    end
                    if bp and equipped < 2 then
                        for _, tool in ipairs(bp:GetChildren()) do
                            if tool.Name == "TriLaserGun" then
                                tool.Parent = char
                                equipped += 1
                                if equipped >= 2 then
                                    break
                    ...
                    task.delay(0.1, function()
                        for _, g in ipairs(char:GetChildren()) do
                            if g.Name == "TriLaserGun" and g:FindFirstChild("Click") then
                                g.Click:FireServer(tt.Position)
                            end
                        end
...
makeBtn(
    ...
    local tf = set(
        Instance.new("Part", workspace),
        {
            ...
            CFrame = targetCF * CFrame.new(0, -4, 0),
            Anchored = true,
            Transparency = 1,
        }
    )
    ...
    if r then
        if env.desyncActive then
            env.fakeCF, env.realCF = targetCF, targetCF
        elseif NullDesyncActive then
            nullSavedCF = targetCF
        else
            r.CFrame = targetCF
        end
    end
...
createToggle(0, -360, "Fly", Color3.fromRGB(40, 100, 150), false, function()
    ...
    if env.desyncActive or NullDesyncActive then
        ...
        if Flying and root and hum then
            local bv = set(
                Instance.new("BodyVelocity", root),
                { Name = "FlyVelocity", MaxForce = Vector3.new(9e9, 9e9, 9e9), Velocity = Vector3.new() }
            )
            local bg = set(
                Instance.new("BodyGyro", root),
                { Name = "FlyGyro", MaxTorque = Vector3.new(9e9, 9e9, 9e9), P = 9e4, CFrame = root.CFrame }
            )
            task.spawn(function()
...
createToggle(0, -330, "Give Gear", Color3.fromRGB(120, 80, 40), false, function()
    GiveDroppedGearActive = not GiveDroppedGearActive
    return GiveDroppedGearActive
end)
...
createToggle(0, -300, "Perm Desync", Color3.fromRGB(40, 100, 150), false, function()
    ...
    do
        local carpet = (
            LP:FindFirstChild("Backpack") and LP.Backpack:FindFirstChild("RainbowMagicCarpet")
        ) or char:FindFirstChild("RainbowMagicCarpet")
        if carpet then
    ...
    RS:BindToRenderStep("DesyncCameraFix", Enum.RenderPriority.Camera.Value - 1, function()
        if root then
            root.CFrame = env.realCF
        end
    end)
    ...
    heartbeatConn = RS.Heartbeat:Connect(function(dt)
        ...
        local sChar
        for t, a in pairs(StealArmTargets) do
            if a and t.Character then
                sChar = t.Character
                break
            end
        end
        if sChar then
            local tArm = sChar:FindFirstChild("Left Arm")
                or sChar:FindFirstChild("LeftLowerArm")
                or sChar:FindFirstChild("LeftHand")
            if tArm then
                env.realCF = CFrame.new(tArm.Position)
                    * cam.CFrame.Rotation
                    * CFrame.new(-1.5, 0, 0)
            end
        end
...
createToggle(0, -240, "Ghost Touch", Color3.fromRGB(150, 80, 20), false, function()
    ...
    local target, root =
        LatestClone or workspace:FindFirstChild(LP.Name .. "'s Clone"),
        LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
        ...
        if ct then
            ...
            if handle then
                for _, j in ipairs(target:GetDescendants()) do
                    if
                        (j:IsA("JointInstance") or j:IsA("WeldConstraint"))
                        and (j.Part0 == handle or j.Part1 == handle)
                    then
                        j:Destroy()
                    end
                end
                for _, p in ipairs(ct:GetDescendants()) do
                    if p:IsA("BasePart") then
...
createToggle(0, -210, "Inf Stack", Color3.fromRGB(100, 40, 100), false, function()
    ...
    if IsStackingActive and carrot and char and char:FindFirstChild("Humanoid") then
        char.Humanoid:EquipTool(carrot)
        task.wait(0.1)
        carrot:Activate()
        task.wait(0.1)
...
local function activateGodMode()
    ...
    task.spawn(function()
        local char, bp = LP.Character, LP:FindFirstChild("Backpack")
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        if not char or not bp or not hum then
            isGodMode = false
            return
        end
        ...
        while tick() - startTime < 10 do
            ...
            if not nO and not nS then
                task.wait(0.1)
            end
        end
        GodBtn.Text = "WIPING HANDS..."
        hum:UnequipTools()
        task.wait(0.4)
        ...
        if lastSkin and lastSkin.Parent == bp then
            table.insert(final, lastSkin)
        end
        for _, t in ipairs(final) do
            t.Parent = char
        end
        task.wait(0.05)
        for _, t in ipairs(final) do
...
PeriBtn.MouseButton1Click:Connect(function()
    ...
    if char then
        for _, t in ipairs(char:GetChildren()) do
            if t.Name == "RainbowPeriastron" and not usedPeris[t] then
                peri = t
                break
            end
        end
    end
    if not peri and bp then
        for _, t in ipairs(bp:GetChildren()) do
            if t.Name == "RainbowPeriastron" and not usedPeris[t] then
                peri = t
                break
    ...
    if not char or not hum or not peri then
        return
    end
    hum:EquipTool(peri)
    task.wait(0.1)
    ...
    if
        pth
        and pth:FindFirstChild("RainbowPeriastron")
        and pth.RainbowPeriastron:FindFirstChild("Server")
        and pth.RainbowPeriastron.Server:FindFirstChild("RainBeam")
    then
    ...
    if peri:FindFirstChild("Remote") then
        peri.Remote:FireServer(Enum.KeyCode.Q)
    end
    usedPeris[peri] = true
end)

local HatDropBtn = makeBtn(
    MF,
    "Hat Drop",
    UDim2.new(0.5, 0, 0, 30),
    UDim2.new(0, 0, 1, -90),
...
HatDropBtn.MouseButton1Click:Connect(function()
    ...
    if hum then
        ...
        task.delay(1, function()
            if HatDropBtn and HatDropBtn.Parent then
                HatDropBtn.Text, HatDropBtn.BackgroundColor3 = oldTxt, oldCol
            end
        end)
    else
        local oldTxt, oldCol = HatDropBtn.Text, HatDropBtn.BackgroundColor3
...
local function getHighlightedPlayers()
    ...
    for uid, a in pairs(RocketTargets) do
        if a then
            local p = P:GetPlayerByUserId(uid)
            if p then
                set[p] = true
            end
        end
    end
    for p, _ in pairs(VoidKillTargets) do
        set[p] = true
    end
    for p, _ in pairs(SKillTargets) do
        set[p] = true
    end
    for p, _ in pairs(SpawnCloneTargets) do
        set[p] = true
    end
    for p, _ in pairs(StealArmTargets) do
        set[p] = true
    end
    for p, _ in pairs(WeldKillTargets) do
        set[p] = true
    end
    for p, _ in pairs(FireInterestTargets) do
        set[p] = true
    end
    local list = {}
...
local function applyFireInterestToPlayer(t)
    ...
    if FireInterestLoops[t.Name] then
        FireInterestLoops[t.Name]:Disconnect()
    end
    FireInterestLoops[t.Name] = RS.Heartbeat:Connect(function()
        if not FireInterestTargets[t] then
            if FireInterestLoops[t.Name] then
                FireInterestLoops[t.Name]:Disconnect()
                FireInterestLoops[t.Name] = nil
            end
            return
        end
        local char = LP.Character
        ...
        local handle = heldTool:FindFirstChild("Handle")
        if not handle then
            return
        end
        ...
        local tp = tc:FindFirstChild("Torso") or tc:FindFirstChild("HumanoidRootPart")
        if not tp then
            return
        end
        pcall(function()
...
createToggle(0, -30, "R-Spam", Color3.fromRGB(150, 30, 30), false, function()
    ...
    if not RocketSpamActive then
        table.clear(RocketTargets)
        table.clear(RocketLastKnownPos)
    else
        for _, p in ipairs(getHighlightedPlayers()) do
            RocketTargets[p.UserId] = true
        end
    end
    refreshPlayerColors()
...
local THB = makeBtn(
    ...
    "TP Home",
    UDim2.new(0.5, 0, 0, 30),
    UDim2.new(0.5, 0, 1, -390),
    Color3.fromRGB(40, 70, 40)
)
THB.MouseButton1Click:Connect(function()
    ...
    if r then
        ...
        if thc then
            ...
            task.delay(1, function()
                if THB.Parent then
                    THB.Text, THB.BackgroundColor3 = old, Color3.fromRGB(40, 70, 40)
                end
            end)
        end
    end
...
createToggle(0.5, -360, "Noclip", Color3.fromRGB(120, 40, 120), false, function()
    NoclipActive = not NoclipActive
    return NoclipActive
end)
createToggle(0.5, -330, "Spawn Clone", Color3.fromRGB(40, 120, 40), false, function()
    SpawnCloneActive = not SpawnCloneActive
    if not SpawnCloneActive then
        table.clear(SpawnCloneTargets)
    end
    refreshPlayerColors()
    return SpawnCloneActive
end)

local TPCloneBtn = makeBtn(
    MF,
    "TP Clone",
    UDim2.new(0.5, 0, 0, 30),
    UDim2.new(0.5, 0, 1, -300),
    Color3.fromRGB(50, 50, 50)
)
TPCloneBtn.TextSize = 11
TPCloneBtn.MouseButton1Click:Connect(function()
    local root = LP.Character and LP.Character:FindFirstChild("HumanoidRootPart")
    local tc = LatestClone or workspace:FindFirstChild(LP.Name .. "'s Clone")
    if root and tc then
        local cT = tc:FindFirstChild("Torso")
            or tc:FindFirstChild("UpperTorso")
            or tc:FindFirstChild("HumanoidRootPart")
        if cT then
            ...
            task.spawn(function()
                for i = 1, 20 do
                    if cT and cT.Parent and root then
                        cT.CFrame = VOID_CF
                        if env.desyncActive then
                            env.realCF = VOID_CF + Vector3.new(0, 3, 0)
                        elseif NullDesyncActive then
                            nullSavedCF = VOID_CF + Vector3.new(0, 3, 0)
                        else
                            root.CFrame = VOID_CF + Vector3.new(0, 3, 0)
                        end
                    end
                    RS.Heartbeat:Wait()
                end
...
createToggle(0.5, -270, "Anti-Pickup", Color3.fromRGB(40, 120, 40), false, function()
    ...
    if AntiPickupActive then
        ...
        antiPickupConnection = workspace.DescendantAdded:Connect(function(o)
            if AntiPickupActive then
                task.wait(0.05)
                hideTool(o)
            end
        ...
        local function handleNewGear(item)
            ...
            if item:IsA("Tool") and not allowedTools[item] then
                task.defer(function()
                    if item and item.Parent then
                        item.Parent = hiddenToolsCache
                    end
                end)
            end
        end
        if char then
            antiPickupCharConn = char.ChildAdded:Connect(handleNewGear)
        end
        if bp then
            antiPickupBpConn = bp.ChildAdded:Connect(handleNewGear)
        end
    else
        ...
        if antiPickupCharConn then
            antiPickupCharConn:Disconnect()
            antiPickupCharConn = nil
        end
        if antiPickupBpConn then
            antiPickupBpConn:Disconnect()
            antiPickupBpConn = nil
        end
...
createToggle(0.5, -240, "Anti-Void", Color3.fromRGB(40, 120, 120), true, function()
    AntiVoidActive = not AntiVoidActive
    workspace.FallenPartsDestroyHeight = AntiVoidActive and -9e9
        or (NullDesyncActive and -9e9 or -500)
    ...
    if not StealArmActive then
        ...
        if hl[1] then
            ...
            task.delay(0.2, function()
                if StealArmTargets[hl[1]] then
                    StealArmTargets[hl[1]] = nil
                    refreshPlayerColors()
                end
            end)
        end
...
local AASB = makeBtn(
    ...
    "Anti-Arm Steal",
    UDim2.new(0.5, 0, 0, 30),
    UDim2.new(0.5, 0, 1, -180),
    Color3.fromRGB(50, 50, 50)
)
AASB.TextSize = 11
local function activateAntiArm()
    if isAntiStealing then
        return
    end
    isAntiStealing, AASB.BackgroundColor3, AASB.Text =
        true, Color3.fromRGB(120, 30, 30), "GLITCHING ARMS..."
    task.spawn(function()
        if LP and LP.Character and LP:FindFirstChild("Backpack") then
            ...
            if iK and iK:IsA("Tool") then
                ...
                local ks = {}
                for _, i in ipairs(bp:GetChildren()) do
                    if i:IsA("Tool") and i.Name == "MadMurdererKnife" then
                        table.insert(ks, i)
                    end
                end
                if #ks >= 2 then
        ...
        task.wait(1.5)
        AASB.BackgroundColor3, AASB.Text, isAntiStealing =
            Color3.fromRGB(50, 50, 50), "Anti-Arm Steal", false
    end)
end
...
local DelGearBtn = makeBtn(
    MF,
    "Delete Gear",
    UDim2.new(0.5, 0, 0, 30),
    UDim2.new(0.5, 0, 1, -90),
    Color3.fromRGB(50, 50, 50)
)
DelGearBtn.TextSize = 11
DelGearBtn.MouseButton1Click:Connect(function()
    ...
    if tool then
        ...
        DelGearBtn.Text, DelGearBtn.BackgroundColor3 =
            "DELETED: " .. nm, Color3.fromRGB(150, 40, 40)
        task.delay(1, function()
            if DelGearBtn and DelGearBtn.Parent then
                DelGearBtn.Text, DelGearBtn.BackgroundColor3 = oldTxt, oldCol
            end
        end)
    else
        local oldTxt, oldCol = DelGearBtn.Text, DelGearBtn.BackgroundColor3
...
createToggle(0.5, -60, "Fire Interest", Color3.fromRGB(200, 50, 200), false, function()
    ...
    if not FireInterestActive then
        for _, l in pairs(FireInterestLoops) do
            if l then
                l:Disconnect()
            end
        end
        table.clear(FireInterestLoops)
        table.clear(FireInterestTargets)
    else
        for _, p in ipairs(getHighlightedPlayers()) do
            applyFireInterestToPlayer(p)
        end
    end
    refreshPlayerColors()
    return FireInterestActive
end)
createToggle(0.5, -30, "Void-Kill", Color3.fromRGB(40, 150, 160), false, function()
    VoidKillActive = not VoidKillActive
    if not VoidKillActive then
        clearToolTargets(VoidKillTargets, VoidKillLoops, VoidKillTrackers, ActiveVoidKillSwords)
    end
    refreshPlayerColors()
    return VoidKillActive
end)
    ...
    if not WeldKillActive then
        ...
        for tp, _ in pairs(WeldKillTargets) do
            if tp and tp.Character then
                local tr = tp.Character:FindFirstChild("Torso")
...
local function E(t, btn)
    ...
    if WeldKillActive then
        ...
        local tRoot = tChar
            and (tChar:FindFirstChild("Torso") or tChar:FindFirstChild("HumanoidRootPart"))
        if tRoot then
            local thc = homeCFrame
            local so = LP:FindFirstChild("Spawn")
            if so and so.Value then
                local sg = workspace:FindFirstChild(tostring(so.Value))
                if sg and sg:FindFirstChild("SpawnLocation") then
                    thc = sg.SpawnLocation.CFrame + Vector3.new(0, 4, 0)
                end
            end
            local baseSpot = (thc or CFrame.new(0, 10, 0)) * CFrame.new(0, -3, 0)
            local wkLoop
            wkLoop = RS.Heartbeat:Connect(function()
                if WeldKillTargets[t] and tRoot and tRoot.Parent then
                    tRoot.Anchored, tRoot.CFrame = true, baseSpot
                else
            ...
            if myHum then
                newHum = myHum:Clone()
                myHum:Destroy()
                newHum.Parent = myChar
                if NullDesyncActive and nullCamPart then
                    workspace.CurrentCamera.CameraSubject = nullCamPart
                else
            ...
            if NullDesyncActive then
                ...
                if th then
                    local dC
                    dC = th.Died:Connect(function()
                        if nullWeldOverride == behindCF then
                            nullWeldOverride = nil
                        end
                        if dC then
                            dC:Disconnect()
            ...
            if
                not env.desyncActive
                and not NullDesyncActive
                and myChar
                and myChar:FindFirstChild("HumanoidRootPart")
            then
                myChar.HumanoidRootPart.CFrame = behindCF
            end
            ...
            task.spawn(function()
                ...
                while tick() < endTime and WeldKillTargets[t] and myChar and newHum do
                    ...
                    if bp then
                        for _, i in ipairs(bp:GetChildren()) do
                            if i:IsA("Tool") and not badGears[i.Name] then
            ...
            local dC
            dC = t.CharacterAdded:Connect(function()
                WeldKillTargets[t] = nil
                refreshPlayerColors()
                if dC then
                    dC:Disconnect()
    ...
    if RocketSpamActive then
        ...
        if WeldKillActive then
            ...
            if not RocketTargets[t.UserId] then
                RocketLastKnownPos[t.UserId] = nil
            end
        end
    end
    ...
    if VoidKillActive then
        ...
        if VoidKillTargets[t] then
            clearToolTargets({ [t] = true }, VoidKillLoops, VoidKillTrackers, ActiveVoidKillSwords)
            VoidKillTargets[t] = nil
        else
            VoidKillTargets[t] = true
            refreshPlayerColors()
            local function doVoidKill()
                if not VoidKillTargets[t] then
                    return
                end
                local char = LP.Character
                local hum, root, bp =
                ...
                if not char or not hum or not root or not bp then
                    return
                end
                ...
                task.wait(0.1)
                hum:UnequipTools()
                local sba = bp:FindFirstChild("Bear Arm")
                    or char:FindFirstChild("Bear Arm")
                    or bp:FindFirstChild("BearArm")
                    or char:FindFirstChild("BearArm")
                if sba then
                    hum:EquipTool(sba)
                    local tArm = t.Character
                        and (
                            t.Character:FindFirstChild("Left Arm")
                            or t.Character:FindFirstChild("LeftHand")
                            or t.Character:WaitForChild("Left Arm", 3)
                            or t.Character:WaitForChild("LeftHand", 3)
                        )
                    if tArm and (env.desyncActive or NullDesyncActive) then
                        ...
                        task.wait(0.4)
                        StealArmTargets[t] = nil
                        env.realCF, Flying = oldReal, wasFly
                    else
                        task.wait(0.4)
                    end
                end
                ...
                for _, s in pairs(all) do
                    if not ActiveVoidKillSwords[s] or ActiveVoidKillSwords[s] == t then
                        sword = s
                        break
                    end
                end
                if sword then
                    ...
                    task.delay(1, function()
                        if btn and btn.Parent and not VoidKillTargets[t] then
                            btn.Text = old
                            refreshPlayerColors()
                        end
                    end)
            ...
            local function hookDeath(tc)
                ...
                if th then
                    th.Died:Connect(function()
                        if VoidKillLoops[t.Name] then
                            VoidKillLoops[t.Name]:Disconnect()
                            VoidKillLoops[t.Name] = nil
                        end
                        for s, ply in pairs(ActiveVoidKillSwords) do
                            if ply == t and s.Parent == LP.Character then
                                s.Parent = LP:FindFirstChild("Backpack") or s.Parent
                            end
                        end
                    end)
                end
            ...
            if t.Character then
                task.spawn(function()
                    hookDeath(t.Character)
                end)
            end
            VoidKillTrackers[t.Name] = t.CharacterAdded:Connect(function(nc)
                hookDeath(nc)
                if VoidKillTargets[t] then
                    task.wait(0.3)
...
CB.MouseButton1Click:Connect(function()
    ...
    if homeTPLoop then
        pcall(function()
            task.cancel(homeTPLoop)
        end)
        homeTPLoop = nil
    end
    AntiPickupActive, isGodMode, VoidKillActive, SpawnCloneActive, FireInterestActive, KillAuraActive =
    ...
    if IsAntiLaser then
        ...
        if laserNames[c.Name] or c:IsA("SelectionPartLasso") then
            if c:IsA("BasePart") then
                c.CanTouch, c.CanCollide, c.Transparency = false, false, 1
            end
            c:Destroy()
        end
    end
    if c.Name == LP.Name .. "'s Clone" then
        LatestClone = c
    end
end)
...
task.spawn(function()
    while true do
        ...
        if InfSupernovaActive and LP then
            ...
            if
                pth
                and pth:FindFirstChild("IvoryPeriastron")
                and pth.IvoryPeriastron:FindFirstChild("Server")
                and pth.IvoryPeriastron.Server:FindFirstChild("StarSummon")
            then
                for _, c in ipairs(pth.IvoryPeriastron.Server.StarSummon:GetChildren()) do
        ...
        homeTPLoop = task.spawn(function()
            for i = 1, 6 do
                ...
                if root and root.Parent then
                    root.CFrame = homeCFrame * CFrame.Angles(0, math.rad(-90), 0)
                end
                task.wait(0.15)
            end
            homeTPLoop = nil
...
task.spawn(function()
    ...
    if root then
        ...
        if not found then
            ...
            if so and so.Value then
                local sg = workspace:FindFirstChild(tostring(so.Value))
                if sg and sg:FindFirstChild("SpawnLocation") then
                    found = sg.SpawnLocation.CFrame + Vector3.new(0, 4, 0)
                end
            end
        end
        if not found then
            ...
            for i = 1, 8 do
                local loc = workspace:FindFirstChild("Spawn" .. i)
                    and workspace:FindFirstChild("Spawn" .. i):FindFirstChild("SpawnLocation")
                if
                    loc
                    and loc:IsA("BasePart")
                    and (loc.Position - root.Position).Magnitude < cd
                then
                    cd, closest = (loc.Position - root.Position).Magnitude, loc
                end
            end
            if closest then
                found = closest.CFrame + Vector3.new(0, 4, 0)
            end
        end
        if found then
            homeCFrame = found
    ...
    if NullDesyncActive and LP.Character then
        task.wait(0.5)
        startNullDesync(LP.Character)
    end
    task.wait(3)
    ...
    for _, v in ipairs(workspace:GetDescendants()) do
        if v.Name == "SpawnWalls" and v:IsA("BasePart") then
            v.CFrame, v.Anchored, v.CanCollide = FAR, true, false
        end
    end
end)
...
task.spawn(function()
    ...
    repeat
        task.wait(0.1)
    until not giveAllRunning -- wait for first to finish
    task.wait(2)
    runGiveAll()
    repeat
        task.wait(0.1)
    until not giveAllRunning -- wait for second to finish
    task.wait(2)
    do
        local char, bp = LP.Character, LP:FindFirstChild("Backpack")
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        local bow = (bp and bp:FindFirstChild("OrnateGoldenBow"))
            or (char and char:FindFirstChild("OrnateGoldenBow"))
        if bow and hum then
            hum:EquipTool(bow)
            task.wait(0.2)
            hum:UnequipTools()
